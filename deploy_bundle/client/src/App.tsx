...
import {
  Activity, BarChart4, Database, FileText, Filter, GitBranch, Layout, LineChart,
  Database as LuceneDatabase, CheckCircle, Play, Plus, Search, Settings, Share2,
  ShieldCheck, Trash2, TrendingUp, Upload, User, Download, Code, Cpu, Zap, Layers,
  PlayCircle, Loader2, Info, ArrowRight, Table, AlertTriangle, RotateCcw
} from "lucide-react";
...
import PowerBiLab from "./PowerBiLab";
import GenAiLab from "./GenAiLab";
...
// ─── Types ───────────────────────────────────────────────────────────────────
type Page = "home" | "data" | "process" | "academic" | "ml" | "report" | "help" | "result" | "db-connect" | "builder" | "python-lab" | "sql-lab" | "power-bi" | "lab-notebooks" | "lab-assignments" | "lab-achievements" | "practical" | "competitions" | "deployment" | "quality-control" | "visualization" | "ml-learning" | "data-learning" | "gen-ai" | "assessment" | "exams";
...
function secureDownload(url: string, filename: string) {
  ...
  axios.get(url, {
    ...
    alert("下载失败：权限不足或文件已移除");
  });
}

function DevelopingPlaceholder({ title, lang }: { title: string; lang: Language }) {
  return (
    <div className="page animate-fade" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", background: "var(--bg-base)" }}>
      <div style={{ fontSize: 64, marginBottom: 24 }}>🚧</div>
      <h2 style={{ fontSize: 24, fontWeight: 800, color: "var(--text-primary)", marginBottom: 8 }}>{title} - {lang === "zh" ? "正在建设中" : "Under Construction"}</h2>
      <p style={{ fontSize: 14, color: "var(--text-muted)", textAlign: "center", maxWidth: 400 }}>
        {lang === "zh" ? "该模块正在紧锣密鼓地开发中，我们将很快为您带来强大的新功能。敬请期待！" : "This module is currently under development. We will bring you powerful new features soon. Stay tuned!"}
      </p>
      <div style={{ marginTop: 32, padding: "12px 24px", borderRadius: 12, background: "var(--accent-dim)", color: "var(--accent)", fontWeight: 700, fontSize: 12, letterSpacing: 1 }}>
        COMING SOON · STATLAB KINETIC
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  // Bridge for ReportPage (which is defined outside App) to change pages
  useEffect(() => {
    (window as any).statlab_set_page = (p: Page) => setPage(p);
    return () => { delete (window as any).statlab_set_page; };
  }, []);
  ...
  const [previewMode, setPreviewMode] = useState<"table" | "coding" | "health">("table");
  ...
  const importSample = useCallback(async (name?: string) => {
    ...
    try {
      const { data } = await axios.post<DatasetInfo>(api, {}, { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
  ...
  const refreshDatasets = useCallback(async () => {
    try {
      const { data } = await axios.get<DatasetInfo[]>("/api/datasets", { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
      setDatasets((prev) => {
  ...
  const refreshRunHistory = useCallback(async () => {
    if (!user?.id) return;
    try {
      const { data: algos } = await axios.get<AlgorithmSpec[]>("/api/algorithms", { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
      const { data: history } = await axios.get<RunHistoryItem[]>("/api/run-history", { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
  ...
  useEffect(() => {
    if (!user?.id) return;
    axios.get<AlgorithmSpec[]>("/api/algorithms", { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } }).then((r) => setAlgorithms(r.data))
    ...
    const restoreDbDatasets = async () => {
      ...
      try {
        ...
        for (const id of cachedIds) {
          try {
            const { data: dbDs } = await axios.get(`/api/connections/dataset/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
  ...
  const run = async () => {
    ...
    try {
      ...
      const historyItem: RunHistoryItem = {
        id: String((data as any).runId || Date.now()),
  ...
  const deleteDataset = async (id: DatasetId, e: React.MouseEvent) => {
    ...
    try {
      addLog("info", "删除数据集...");
      await axios.delete(`/api/datasets/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
      setDatasets((prev) => {
  ...
  const loadFullData = async () => {
    ...
    try {
      const { data } = await axios.get(`/api/datasets/version/${activeVersion.id}/full-data`, { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
  ...
  const deleteVersion = async (id: DatasetId) => {
    ...
    try {
      await axios.delete(`/api/datasets/version/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
      setDatasets((prev) => {
        return prev.map((ds) => {
  ...
  const onBatchDeleteReport = async (ids: string[]) => {
    ...
    try {
      await axios.post("/api/run-history/batch-delete", { ids }, { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
  ...
  const dataCenterItems: { id: Page; icon: string; labelZh: string; labelEn: string; isPlaceholder?: boolean }[] = [
    { id: "data", icon: "🗂️", labelZh: "我的数据", labelEn: "My Data" },
    { id: "process", icon: "🛠️", labelZh: "数据处理", labelEn: "Data Process" },
    { id: "academic", icon: "📐", labelZh: "统计分析", labelEn: "Statistical Analysis" },
    { id: "ml", icon: "🤖", labelZh: "机器学习", labelEn: "Machine Learning" },
    { id: "builder", icon: "📽️", labelZh: "可视化中心", labelEn: "Visualization Center" },
  ];

  const learningItems: { id: Page; icon: string; labelZh: string; labelEn: string; isPlaceholder?: boolean }[] = [
    { id: "python-lab", icon: "🧠", labelZh: "机器学习", labelEn: "Machine Learning" },
    { id: "sql-lab", icon: "📊", labelZh: "数据处理 (SQL)", labelEn: "SQL Training" },
    { id: "power-bi", icon: "📈", labelZh: "Power BI 学习", labelEn: "Power BI Learning" },
    { id: "practical", icon: "⚔️", labelZh: "实战训练", labelEn: "Practical Training" },
    { id: "gen-ai", icon: "✨", labelZh: "生成式 AI", labelEn: "Generative AI" },
    { id: "lab-assignments", icon: "📝", labelZh: "作业反馈", labelEn: "Assignments" },
  ];

  const certificationItems: { id: Page; icon: string; labelZh: string; labelEn: string; isPlaceholder?: boolean }[] = [
    { id: "assessment", icon: "🎯", labelZh: "能力评估", labelEn: "Assessment", isPlaceholder: true },
    { id: "exams", icon: "📜", labelZh: "等级考试", labelEn: "Exams", isPlaceholder: true },
    { id: "lab-achievements", icon: "🏆", labelZh: "我的证书", labelEn: "Certificates" },
  ];
    ...
    <ConfigProvider locale={getAntdLocale()}>
      <div className={`shell ${isSidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
        ...
        <aside className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''}`}>
          <div className="sidebar-logo brand-logo" title={isSidebarCollapsed ? "Continental StatLab" : undefined}>
            <div className="brand-mark">
              <img src="/continental_logo.png" alt="Continental" />
            </div>
            {!isSidebarCollapsed && (
              <div className="brand-copy">
                <div className="brand-title">Continental</div>
                <div className="brand-sub">StatLab Kinetic</div>
              </div>
            )}
          </div>

          <nav className="sidebar-nav">
            <div className={`sidebar-item ${page === "home" ? "active" : ""}`} onClick={() => setPage("home")} title={isSidebarCollapsed ? (lang === "zh" ? "首页概览 Overview" : "Overview") : undefined}>
              <span className="nav-icon" style={{ fontSize: 16 }}>🏠</span>
              {!isSidebarCollapsed && (
                <div className="sidebar-item-labels">
                  <span className="label-zh">首页概览</span>
                  <span className="label-en">Overview</span>
                </div>
              )}
              {page === "home" && <div className="active-indicator" />}
            </div>

            {/* Data Analysis Center */}
            {!isSidebarCollapsed ? (
              <div className="sidebar-section-title" style={{ marginTop: 16 }}>数据分析中心 Analysis Center</div>
            ) : (
              <div className="sidebar-divider" />
            )}
            {dataCenterItems.map((item) => (
              <div key={item.id} className={`sidebar-item ${page === item.id ? "active" : ""}`} onClick={() => setPage(item.id)} title={isSidebarCollapsed ? `${item.labelZh} ${item.labelEn}` : undefined}>
                <span className="nav-icon" style={{ fontSize: 16 }}>{item.icon}</span>
                {!isSidebarCollapsed && (
                  <div className="sidebar-item-labels">
                    <span className="label-zh">{item.labelZh}</span>
                    <span className="label-en">{item.labelEn}</span>
                  </div>
                )}
                {(item.id === "academic" || item.id === "ml") && !isSidebarCollapsed && (
            ...
            {/* Learning Center */}
            {!isSidebarCollapsed ? (
              <div className="sidebar-section-title" style={{ marginTop: 16 }}>学习中心 Learning Center</div>
            ) : (
              <div className="sidebar-divider" />
            )}
            ...
            {!isSidebarCollapsed ? (
              <div className="sidebar-section-title" style={{ marginTop: 16 }}>能力认证 Certification</div>
            ) : (
              <div className="sidebar-divider" />
            )}
            ...
            {!isSidebarCollapsed && datasets.length > 0 && (
              <>
                <div className="sidebar-section-title" style={{ marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span>我的数据 My Data</span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <button className="btn-icon-only" title="Select or clear all" onClick={selectAllDeletableDatasets} style={{ opacity: 0.75 }}>
                      <Table size={14} />
                    </button>
                    <button className="btn-icon-only" title="Upload" onClick={() => setShowUploadModal(true)} style={{ opacity: 0.6 }}>
                      <Plus size={14} />
                    </button>
                  </div>
                </div>
                <div style={{ maxHeight: "calc(100vh - 450px)", overflowY: "auto", paddingRight: 4, marginTop: 4 }}>
                  {datasets.map((d) => (
                    <div key={d.id} className={`sidebar-item ${activeDatasetId === d.id ? "active" : ""}`}
                      style={{ padding: '6px 12px', margin: '2px 8px', fontSize: 12, gap: 8 }}
                      title={d.name}
                      onClick={() => { setActiveDatasetId(d.id); if (d.versions?.[0]) setActiveVersion(d.versions[0] as any); setPage("data"); }}>
                      <span style={{ fontSize: 13, opacity: 0.7 }}>{(d as any).sourceType === 'db' ? '🗄️' : '📄'}</span>
          ...
          <div className="sidebar-footer" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <button className="sidebar-collapse-btn" onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} title={isSidebarCollapsed ? "Expand" : "Collapse"}>
        ...
        <div className="main">
          ...
          {page === "builder" ? (
            ...
            <>
                ...
                <div className="home-page animate-fade" style={{ flex: 1, overflowY: "auto", position: "relative", zIndex: 1 }}>
                  ...
                  <div style={{ padding: "0 60px 60px", maxWidth: 1400, margin: "0 auto", position: "relative", zIndex: 10 }}>
                    <div className="home-stats-v2" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24, marginTop: -40 }}>
                      <div className="card sparkline-bg" style={{ padding: 24, borderRadius: 20, boxShadow: "0 20px 40px rgba(0,0,0,0.08)", border: "1px solid var(--border)", background: "white", transition: "transform 0.2s" }}>
                        <div style={{ color: "var(--accent)", marginBottom: 12 }}><Database size={28} /></div>
                      ...
                      <div className="card sparkline-bg" style={{ padding: 24, borderRadius: 20, boxShadow: "0 20px 40px rgba(0,0,0,0.08)", border: "1px solid var(--border)", background: "white", transition: "transform 0.2s" }}>
                        <div style={{ color: "var(--success)", marginBottom: 12 }}><BarChart4 size={28} /></div>
                      ...
                      <div className="card sparkline-bg" style={{ padding: 24, borderRadius: 20, boxShadow: "0 20px 40px rgba(0,0,0,0.08)", border: "1px solid var(--border)", background: "white", transition: "transform 0.2s" }}>
                        <div style={{ color: "var(--purple)", marginBottom: 12 }}><Layers size={28} /></div>
                      ...
                      <div className="card sparkline-bg" style={{ padding: 24, borderRadius: 20, boxShadow: "0 20px 40px rgba(0,0,0,0.08)", border: "1.5px solid rgba(255,154,0,0.2)", background: "white", transition: "transform 0.2s" }}>
                        <div style={{ color: "#ff9a00", marginBottom: 12 }}><Cpu size={28} /></div>
                ...
                <div className="topbar">
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div className="topbar-title">
                      {(() => {
                        const allItems = [...dataCenterItems, ...learningItems, ...certificationItems];
                        const found = allItems.find((n) => n.id === page);
                        if (found) return <>{found.icon} {found.labelZh} {found.labelEn}</>;
                        return null;
                      })()}
                    </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 8px', borderRadius: 8, background: 'var(--bg-raised)' }}>
                      <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', lineHeight: 1.1 }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)' }}>{user?.name || user?.username}</span>
                    ...
                    <div className="topbar-actions">
                      {activeDataset && (
                        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                          ...
                          {activeVersion && (
                            <button className="btn btn-secondary btn-sm" onClick={() => secureDownload(`/api/datasets/version/${activeVersion.id}/download`, `${activeDataset.name}_V${activeVersion.version}.csv`)} title="导出当前版本为 CSV">
                              <Download size={14} /> 导出
                            </button>
                          )}
              ...
              {/* ── 我的数据 ── */}
              {page === "data" && (
                <div className="page" style={{ padding: 0, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                  <DataPage
                    datasets={datasets as any}
                    activeDataset={activeDataset as any}
                    activeVersion={activeVersion as any}
                    activeDatasetId={activeDatasetId as any}
                    setActiveDatasetId={setActiveDatasetId as any}
                    setActiveVersion={setActiveVersion as any}
                    addLog={addLog}
                    setShowUploadModal={setShowUploadModal}
                    setPage={setPage}
                    importSample={importSample as any}
                  />
                </div>
              )}
              ...
              {page === "db-connect" && (
                <DbConnect
                  ...
                  onDatasetCreated={async (datasetId) => {
                    try {
                      const res = await axios.get(`/api/connections/dataset/${datasetId}`, { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
                      const dbDs = res.data;
                      const createdAt = new Date().toISOString();
                      const version: VersionLike = {
                        id: dbDs.id,
              ...
              {/* ── 机器学习工作台 ── */}
              {page === "ml" && (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", height: "100%", width: "100%", minHeight: 0 }}>
                  <MLWorkspace
                    algorithms={algorithms}
                          ...
                          return (
                            <div key={sub} className="algo-group">
                              <div className="subcategory-label" onClick={() => setCollapsedGroups(prev => {
                                ...
                                <span>{sub}</span>
                                <span style={{ fontSize: 10, transform: isCollapsed ? "rotate(-90deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>▼</span>
                              </div>
                    ...
                    <div className="analysis-main-workspace">
                      {!selectedAlgo ? (
                        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "var(--text-muted)", opacity: 0.5 }}>
                          <LuceneDatabase size={64} strokeWidth={1} style={{ marginBottom: 16 }} />
                          <div style={{ fontSize: 16, fontWeight: 500 }}>请从左侧算法库选择一个分析方法</div>
                          <div style={{ fontSize: 13, marginTop: 8 }}>Select an algorithm to begin your analysis</div>
                        </div>
                      ) : (
                        <>
                          <div className="workspace-header">
                            <div className="workspace-title-row">
                              ...
                              <Tooltip title={selectedAlgo.explanation || selectedAlgo.description}>
                          ...
                          <div className="workspace-section-title">变量配置 Variable Selection</div>
                          ...
                          <div className="workspace-section-title">分析元数据 Analysis Metadata</div>
                          <div className="analysis-metadata-card">
                            <div className="metadata-grid">
                              <div className="metadata-field">
                                <label className="metadata-label">分析名称 Analysis Title</label>
                                <input
                                  className="param-input"
                                  placeholder="例如: 相关性分析_2024Q1..."
                                  value={analysisName}
                                  onChange={e => setAnalysisName(e.target.value)}
                                />
                              </div>
                              <div className="metadata-field">
                                <label className="metadata-label">分析人 Analyst</label>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 12px', background: 'var(--bg-raised)', borderRadius: '8px', border: '1px solid var(--border)', height: 38 }}>
                                  <div style={{
                                  ...
                                  <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)' }}>{user?.name || user?.username}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="workspace-action-bar">
                            ...
                            <div className="action-buttons">
                              <button className="btn btn-secondary" onClick={() => { setAssignments({}); setResult(null); setAnalysisName(""); }}>重置 Reset</button>
                              <button
                                className="btn btn-primary"
                                style={{ padding: "8px 32px" }}
                                disabled={!isRunReady || running}
                                onClick={run}
                              >
                                {running ? <Loader2 className="animate-spin" size={16} /> : "开始分析 Run Analysis"}
                              </button>
                            </div>
                          </div>
                        </>
                ...
                <ErrorBoundary>
                  <ReportPage
                    ...
                    onDeleteReport={async (id) => {
                      await axios.delete(`/api/run-history/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
                      await refreshRunHistory();
                    }}
              ...
              {page === "gen-ai" && <DevelopingPlaceholder title={lang === "zh" ? "生成式 AI" : "Generative AI"} lang={lang} />}
              {page === "assessment" && <DevelopingPlaceholder title={lang === "zh" ? "能力评估" : "Assessment"} lang={lang} />}
              {page === "exams" && <DevelopingPlaceholder title={lang === "zh" ? "等级考试" : "Professional Exams"} lang={lang} />}
          ...
          <button
            className="side-fab guide-side-fab"
            style={{ top: 'calc(50% + 10px)' }}
            onClick={() => setShowGuide(true)}
            title="新手引导"
          >
            <span style={{ fontSize: 16 }}>✨</span>
            <span style={{ fontSize: 10, fontWeight: 700, marginTop: -2 }}>引导</span>
          </button>
          {showGuide && (
            <GuideOverlay
              tab={guideTab}
...
function DraggableVarItem({ name, type }: { name: string; type: string }) {
  ...
  return (
    <div ref={setNodeRef} {...listeners} {...attributes}
      className="var-item-draggable"
      style={{ ...style, opacity: isDragging ? 0.4 : 1, padding: "10px 14px" }}
    >
      <span style={{ fontSize: 18 }}>{isNum ? '📈' : '🏷️'}</span>
      <div className="var-name" style={{ fontWeight: 600, fontSize: 14 }}>{name}</div>
      <span className={`var-tag ${isNum ? 'numeric' : 'category'}`} style={{ fontSize: 10 }}>{isNum ? '定量' : '定类'}</span>
    </div>
  );
}
...
function HybridSlot({ spec, assigned, allColumns, varSearch, onChange }: {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div className="slot-header" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <span style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>{spec.label}</span>
        <span className={`type-tag ${spec.acceptedTypes.includes('numeric') ? 'numeric' : 'categorical'}`} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 6, fontWeight: 800 }}>
          {spec.acceptedTypes.map(t => t === 'numeric' ? '定量' : '定类').join(' / ')}
        </span>
        {spec.min > 0 && assigned.length === 0 && <span style={{ color: "var(--danger)", fontSize: 11, fontWeight: 700, background: "var(--danger-dim)", padding: "1px 6px", borderRadius: 4 }}>必填</span>}
        {hasMax && <span style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 500 }}>最大容量: {spec.max}</span>}
        <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
          {compatible.length > 0 && !atMax && (
            <button className="btn btn-ghost btn-xs" style={{ fontSize: 11, fontWeight: 700, height: 24, minHeight: 24 }} onClick={fillAll}>全选</button>
          )}
          {assigned.length > 0 && (
            <button className="btn btn-ghost btn-xs" style={{ fontSize: 11, fontWeight: 700, height: 24, minHeight: 24, color: "var(--text-muted)" }} onClick={() => onChange([])}>清空</button>
          )}
        </div>
      </div>
...
// ─── TableInsight ─────────────────────────────────────────────────────────────
function TableInsight({ table, algoName }: { table: { title: string; columns: string[]; rows: (string | number | null)[][] }; algoName: string }) {
...
function DataProcessPage({ activeVersion, activeDatasetId, addLog, onVersionCreated }: {
  ...
  const execute = async () => {
    ...
    try {
      ...
      const { data: freshVer } = await axios.get<DatasetVersionMeta>(`/api/datasets/${activeDatasetId}/versions/${data.versionId}`, { headers: { Authorization: `Bearer ${localStorage.getItem('statlab_token')}` } });
...
function ReportPage({ runHistory, datasets, algorithms, onLoadResult, onDeleteReport, onBatchDeleteReport }: {
  ...
  const [shareModalFor, setShareModalFor] = useState<RunHistoryItem | null>(null);
  const [shareNote, setShareNote] = useState("");
  const [sharing, setSharing] = useState(false);
  const [sharePublicIds, setSharePublicIds] = useState<Set<number>>(new Set());
  const [galleryReports, setGalleryReports] = useState<any[]>([]);
  const [galleryLoading, setGalleryLoading] = useState(false);
  const [fullPreviewData, setFullPreviewData] = useState<any[] | null>(null);
  const [fullPreviewColumns, setFullPreviewColumns] = useState<string[]>([]);
  const [previewLoading, setPreviewLoading] = useState(false);
  const token = localStorage.getItem('statlab_token') || '';

  const onExportCode = (r: RunHistoryItem) => {
    const algo = algorithms.find(a => a.id === r.algoId);
    if (!algo || !r.result) return;

    // Core logic from MLWorkspace
    const params = (r.result as any).params || {};
    const assignments = (r.result as any).assignments || {};
    const target = assignments.target?.[0] || assignments.y?.[0] || "";
    const features = assignments.features || assignments.vars || [];
    const currentVersionId = r.datasetVersionId || (r.result as any).datasetId;

    let code = `\"\"\"
StatLab Machine Learning Export (From History)
Algorithm: ${algo.name} (${algo.id})
Report ID: ${r.id}
\"\"\"
import pandas as pd
import numpy as np
...
from sklearn.model_selection import train_test_split\n`;
    ...
    if (algo.id.includes('xgboost')) code += "import xgboost as xgb\n";
    else if (algo.id.includes('lightgbm')) code += "import lightgbm as lgb\n";
    else if (algo.id.includes('randomForest')) code += "from sklearn.ensemble import RandomForestRegressor, RandomForestClassifier\n";
    else if (algo.id.includes('logistic')) code += "from sklearn.linear_model import LogisticRegression\n";

    code += `\n# 1. Load Data\ndf = pd.DataFrame(dataset)\n`;
    code += `\n# 2. Select variables\ntarget = "${target}"\nfeatures = ${JSON.stringify(features)}\n`;
    code += `X = df[features]\ny = df[target]\n`;
    code += `\n# 3. Handle categorical encoding\nX = pd.get_dummies(X, drop_first=True)\n`;
    code += `\n# 4. Train/Test Split\nX_train, X_test, y_train, y_test = train_test_split(X, y, test_size=${params.testSize || 0.2}, random_state=42)\n`;
    code += `\n# 5. Initialize & Train Model\n`;

    if (algo.id === 'ml.xgboostReg') code += `model = xgb.XGBRegressor(n_estimators=${params.trees || 100}, learning_rate=0.1)\n`;
    else if (algo.id === 'ml.randomForest') code += `model = RandomForestRegressor(n_estimators=${params.trees || 100})\n`;
    else if (algo.id === 'ml.logistic') code += `model = LogisticRegression()\n`;
    else code += `model = Ridge(alpha=1.0)\n`;

    code += `model.fit(X_train, y_train)\n`;
    code += `\n# 6. Evaluation\nprint(f"Train Score: {model.score(X_train, y_train)}")\nprint(f"Test Score: {model.score(X_test, y_test)}")\n`;

    localStorage.setItem('statlab_python_export', code);
    localStorage.setItem('statlab_python_export_ds', String(currentVersionId));

    // Navigate to Python Lab (Assume setPage is available via props or context, but here ReportPage is in App.tsx scope)
    // Actually ReportPage doesn't have setPage, but it's defined inside App.tsx usually? 
    // No, it's a separate function. We need to pass setPage down.
    (window as any).statlab_set_page?.("python-lab");
  };

  const onExportModel = (r: RunHistoryItem) => {
    if (!r.result) return;
    const blob = new Blob([JSON.stringify(r.result, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `model_${r.id}_${r.algoId}.json`;
    a.click();
  ...
  const refreshShareStatus = async () => {
    try {
      const { data } = await axios.get('/api/public/reports');
      setSharePublicIds(new Set(data.map((r: any) => r.id)));
    } catch (err) { console.error("Failed to refresh share status", err); }
  };
  ...
  const loadGallery = async () => {
    ...
    try {
      const { data } = await axios.get('/api/public/reports');
      setGalleryReports(data);
    } catch (err) { console.error("Failed to load gallery", err); }
    finally { setGalleryLoading(false); }
  };

  const shareReport = async (report: RunHistoryItem, isPublic: boolean, note?: string) => {
    setSharing(true);
    try {
      const resp = await axios.post(`/api/run/${report.id}/share`, {
        note: note || "",
        isPublic: !!isPublic
      }, {
        headers: { Authorization: `Bearer ${token}` }
      ...
      alert("分享操作失败: " + (err.response?.data?.error || err.message));
    }
  ...
  const getDatasetName = (r: RunHistoryItem) => {
    ...
    const ds = datasets.find(d => String(d.id) === String(r.datasetId));
    return ds ? ds.name : "未知数据集";
  };

  const loadFullPreview = async (datasetId: any) => {
    setPreviewLoading(true);
    try {
      const { data } = await axios.get(`/api/datasets/${datasetId}/preview`, { headers: { Authorization: `Bearer ${token}` } });
      if (data.rows && data.rows.length > 0) {
        setFullPreviewColumns(Object.keys(data.rows[0]));
        setFullPreviewData(data.rows);
      }
    } catch (err) { alert("此数据集已移动或无法加载"); }
    finally { setPreviewLoading(false); }
  };

  const mlRuns = runHistory.filter(r => r.algoCategory === 'ml');

  return (
    <div className="report-page animate-fade" style={{ background: "var(--bg-base)", minHeight: "100%", padding: "24px 32px" }}>

      {/* Share Modal */}
      {shareModalFor && (
        <div className="modal-overlay" style={{ display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShareModalFor(null)}>
          <div className="card animate-slide-up" style={{ width: 400, padding: 24, margin: 0, border: "1px solid var(--accent-dim)" }} onClick={e => e.stopPropagation()}>
            <div className="card-title" style={{ display: "flex", alignItems: "center", gap: 10, borderBottom: "none", paddingBottom: 0 }}>
              <span style={{ fontSize: 18 }}>🌐 分享设置</span>
            </div>
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 18 }}>您可以将分析结果推送到 StatLab 资源共享平台。</div>

            <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 4 }}>📋 {shareModalFor.analysisName || shareModalFor.algoName}</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 14 }}>{getDatasetName(shareModalFor)} · {shareModalFor.ts}</div>

            <label style={{ fontSize: 12, fontWeight: 700, display: "block", marginBottom: 4, color: "var(--text-secondary)" }}>备注 (可选)</label>
            <input value={shareNote} onChange={e => setShareNote(e.target.value)}
              placeholder="例如：成品率关键因子识别模型"
              style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--bg-raised)", fontSize: 13, color: "var(--text-primary)", marginBottom: 18, boxSizing: "border-box" }} />

            <div style={{ display: "flex", gap: 10 }}>
              <button className="btn btn-primary" style={{ flex: 1 }} disabled={sharing} onClick={() => shareReport(shareModalFor, true, shareNote)}>
                {sharing ? "分享中..." : "🌐 公开分享"}
              </button>
              {sharePublicIds.has(Number(shareModalFor.id)) && (
                <button className="btn btn-danger" disabled={sharing} onClick={() => shareReport(shareModalFor, false)}>取消分享</button>
              )}
              <button className="btn btn-ghost" onClick={() => setShareModalFor(null)}>取消</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Full Preview Modal */}
      {fullPreviewData && (
        <div className="modal-overlay" onClick={() => setFullPreviewData(null)} style={{ background: "rgba(0,0,0,0.8)", backdropFilter: "blur(4px)" }}>
          <div className="modal-content animate-slide-up" style={{ width: "95vw", height: "90vh", padding: 0, overflow: "hidden" }} onClick={e => e.stopPropagation()}>
            <div className="modal-header" style={{ borderBottom: "1px solid var(--border)" }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 800 }}>📂 数据集全量样本 (100行)</div>
                <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{activeReport?.datasetName || "数据集"}</div>
              </div>
              <button className="btn btn-secondary btn-sm" onClick={() => setFullPreviewData(null)}>关闭</button>
            </div>
            <div className="data-table-wrap" style={{ flex: 1, maxHeight: "none", border: "none" }}>
              <table className="data-table">
                <thead>
                  <tr>
                    <th style={{ width: 40, textAlign: "center" }}>#</th>
                    {fullPreviewColumns.map(c => <th key={c}>{c}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {fullPreviewData.map((row, i) => (
                    <tr key={i}>
                      <td style={{ textAlign: "center", fontSize: 10, color: "var(--text-muted)" }}>{i + 1}</td>
                      {fullPreviewColumns.map(c => <td key={c} style={{ fontSize: 12 }}>{String(row[c] ?? "")}</td>)}
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="section-head" style={{ marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <h1 style={{ margin: 0, fontSize: 28, fontWeight: 900, color: "var(--text-primary)" }}>报告与共享中心</h1>
            </div>
            <div style={{ fontSize: 14, color: "var(--text-muted)", marginTop: 4 }}>回溯您的分析记录，或从公共广场获取灵感</div>
          </div>

          <div className="stat-grid" style={{ marginBottom: 0 }}>
            <div className="stat-tile" style={{ padding: "8px 16px" }}>
              <div className="stat-tile-val" style={{ fontSize: 18 }}>{runHistory.length}</div>
              <div className="stat-tile-label" style={{ fontSize: 10 }}>总记录</div>
            </div>
            <div className="stat-tile" style={{ padding: "8px 16px" }}>
              <div className="stat-tile-val" style={{ fontSize: 18 }}>{mlRuns.length}</div>
              <div className="stat-tile-label" style={{ fontSize: 10 }}>工业/ML</div>
            </div>
          </div>
        </div>

        <div style={{ display: "flex", gap: 8, marginBottom: 24, borderBottom: "1px solid var(--border)", paddingBottom: 16 }}>
          {(['mine', 'gallery'] as const).map(tab => (
            <button key={tab}
              className={`btn ${activeTab === tab ? 'btn-primary' : 'btn-ghost'}`}
              style={{ borderRadius: 12, padding: "8px 24px", fontWeight: 700 }}
              onClick={() => setActiveTab(tab)}>
              {tab === 'mine' ? '📋 我的分析报告' : '🌐 StatLab 共享广场'}
              {tab === 'gallery' && sharePublicIds.size > 0 && <span className="tag success" style={{ marginLeft: 8, fontSize: 10 }}>{sharePublicIds.size} 项</span>}
            </button>
          ))}
        </div>
          ...
          <div className="card" style={{ padding: 0, overflow: "hidden", minHeight: 600 }}>
            {activeTab === 'gallery' ? (
              <div className="gallery-section">
                {galleryLoading ? (
                  <div className="empty-state"><div className="empty-title">正在载入资源...</div></div>
                ) : galleryReports.length === 0 ? (
                  <div className="empty-state">
                    <div className="empty-icon">🍃</div>
                    <div className="empty-title">暂时没有公开共享</div>
                    <div className="empty-desc">在「我的报告」中点击 🌐 图标即可发布到此处</div>
                  </div>
                ) : (
                  <div style={{ padding: 16, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
                    {galleryReports.map(rpt => (
                      <div key={rpt.id} className="card hover-lift" style={{ margin: 0, padding: 16, cursor: "pointer", border: "1px solid var(--border)" }}
                        onClick={async () => {
                          setGalleryLoading(true);
                          try {
                            const { data } = await axios.get(`/api/public/reports/${rpt.id}`);
                            const mapped: RunHistoryItem = {
                              ...data,
                              ts: new Date(data.sharedAt || data.createdAt).toLocaleString(),
                              status: 'completed'
                            };
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
                          <span className="tag purple" style={{ fontSize: 9 }}>{rpt.algoCategory || "ANALYSIS"}</span>
                          <span style={{ fontSize: 10, color: "var(--text-muted)" }}>{new Date(rpt.sharedAt || rpt.createdAt).toLocaleDateString()}</span>
                        </div>
                        <h4 style={{ margin: "0 0 8px", fontSize: 15, fontWeight: 800 }}>{rpt.analysisName || rpt.algoName}</h4>
                        <p style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 16, minHeight: 40, lineHeight: 1.5 }}>
                          {rpt.sharedNote || "该分析师很神秘，没有留下备注。"}
                        </p>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, paddingTop: 12, borderTop: "1px dashed var(--border)" }}>
                          <div style={{ width: 24, height: 24, borderRadius: 12, background: "var(--accent)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 900 }}>
                            {(rpt.analystName || "U")[0]}
                          </div>
                          <span style={{ fontSize: 11, fontWeight: 700 }}>{rpt.analystName || "匿名"}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="history-list-wrapper">
                {selectedIds.size > 0 && (
                  <div className="selection-bar animate-slide-down" style={{ background: "var(--bg-raised)", padding: "10px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center" }}>
                    <span style={{ fontSize: 12, fontWeight: 600 }}>已选中 {selectedIds.size} 项报告</span>
                    <button className="btn btn-danger btn-sm" style={{ marginLeft: "auto" }} onClick={() => { if (confirm("确认批量删除？")) { onBatchDeleteReport(Array.from(selectedIds)).then(() => setSelectedIds(new Set())); } }}>批量删除</button>
                  </div>
                )}
                ...
                <table className="data-table" style={{ border: "none" }}>
                  <thead>
                    <tr>
                      <th style={{ width: 40 }}><input type="checkbox" onChange={(e) => {
                        if (e.target.checked) setSelectedIds(new Set(runHistory.map(r => r.id)));
                  ...
                  <tbody>
                    {runHistory.length === 0 && (
                      <tr><td colSpan={5} style={{ textAlign: "center", padding: 80, color: "var(--text-muted)" }}>暂无历史运行报告</td></tr>
                    )}
                    {runHistory.map((r) => (
                      <tr key={r.id} className={activeReport?.id === r.id ? 'active' : ''} onClick={() => setActiveReport(r)} style={{ cursor: "pointer" }}>
                        <td><input type="checkbox" checked={selectedIds.has(r.id)} onClick={e => e.stopPropagation()} onChange={() => {
                          const n = new Set(selectedIds);
                          n.has(r.id) ? n.delete(r.id) : n.add(r.id);
                          setSelectedIds(n);
                        }} /></td>
                        <td>
                          <div style={{ fontWeight: 700, fontSize: 14 }}>{r.analysisName || r.algoName}</div>
                          <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{r.algoName}</div>
                        </td>
                        <td>{getDatasetName(r)}</td>
                        <td>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <div style={{ width: 6, height: 6, borderRadius: 3, background: r.status === 'completed' ? 'var(--success)' : 'var(--danger)' }} />
                            <span style={{ fontSize: 12 }}>{r.status === 'completed' ? '成功' : '失败'}</span>
                          </div>
                          <div style={{ fontSize: 10, color: "var(--text-muted)" }}>{r.ts}</div>
                        </td>
                        <td>
                          <div style={{ display: "flex", gap: 6 }}>
                            <button className="btn-icon-only" style={{ color: sharePublicIds.has(Number(r.id)) ? "var(--accent)" : "var(--text-muted)" }}
                              onClick={(e) => { e.stopPropagation(); setShareNote(r.analysisName || ""); setShareModalFor(r); }} title="分享到广场">
                              <Zap size={14} fill={sharePublicIds.has(Number(r.id)) ? "currentColor" : "none"} />
                            </button>
                            <button className="btn-icon-only text-danger" onClick={(e) => { e.stopPropagation(); if (confirm("确定删除该报告？")) onDeleteReport(r.id); }} title="删除">
                              <Trash2 size={14} />
                            </button>
                          </div>
                        </td>
          ...
          <div className="card" style={{ padding: 24, position: "sticky", top: 20 }}>
            {activeReport ? (
              <div className="animate-fade">
                <div style={{ marginBottom: 20 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                    <div className="tag accent" style={{ fontSize: 10 }}>{activeReport.algoCategory?.toUpperCase() || "REPORT"}</div>
                    <div style={{ display: "flex", gap: 8 }}>
                      {sharePublicIds.has(Number(activeReport.id)) && <span className="tag success" title="已发布到广场">🌐 已共享</span>}
                      <button className="btn-icon-only" onClick={() => setActiveReport(null)}>×</button>
                    </div>
                  </div>
                  <h2 style={{ fontSize: 20, fontWeight: 900, margin: "0 0 4px" }}>{activeReport.analysisName || activeReport.algoName}</h2>
                  <div style={{ fontSize: 12, color: "var(--text-muted)" }}>ID: {activeReport.id} · {activeReport.ts}</div>
                </div>

                {/* Dataset Preview Top Header - Only for user's own reports */}
                {activeTab === 'mine' && activeReport.datasetId && (
                  <div style={{
                    background: "var(--bg-raised)", borderRadius: 12, padding: "14px 18px",
                    border: "1px solid var(--border)", marginBottom: 24,
                    display: "flex", alignItems: "center", gap: 12
                  }}>
                    <div style={{ background: "var(--accent-dim)", color: "var(--accent)", width: 40, height: 40, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Table size={20} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11, fontWeight: 800, color: "var(--text-muted)" }}>关联数据集预览 (Row 1-100)</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }} className="truncate">{activeReport.datasetName || "原数据"}</div>
                    </div>
                    <button className="btn btn-secondary btn-sm" style={{ padding: "6px 10px" }}
                      onClick={() => loadFullPreview(activeReport.datasetId)} disabled={previewLoading}>
                      {previewLoading ? <Loader2 size={14} className="animate-spin" /> : <Layers size={14} />}
                    </button>
                  </div>
                )}

                {activeReport.status === 'completed' && activeReport.result ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>

                    <div className="card" style={{ margin: 0, padding: 20, background: "var(--bg-soft)", border: "none" }}>
                      <div className="section-head" style={{ fontSize: 13, marginBottom: 12 }}>📊 核心指标概览</div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        {activeReport.result.tables?.[0]?.rows.slice(0, 4).map((row, i) => (
                          <div key={i} style={{ background: "var(--bg-surface)", padding: 12, borderRadius: 10, border: "1px solid var(--border)" }}>
                            <div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 2 }}>{String(row[0] || "指标")}</div>
                            <div style={{ fontSize: 16, fontWeight: 800, color: "var(--accent)" }}>{String(row[1] || "-")}</div>
                          </div>
                        ))}
                      </div>
                    </div>

                    <button className="btn btn-primary btn-lg" style={{ width: "100%", padding: "14px", borderRadius: 14 }}
                      onClick={() => onLoadResult(activeReport)}>
                      查看完整详细报告
                    </button>

                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      <button className="btn btn-secondary" style={{ padding: "10px", borderRadius: 10, fontSize: 13, gap: 6 }}
                        onClick={() => onExportCode(activeReport)}>
                        💾 生成代码
                      </button>
                      <button className="btn btn-secondary" style={{ padding: "10px", borderRadius: 10, fontSize: 13, gap: 6 }}
                        onClick={() => onExportModel(activeReport)}>
                        📥 导出模型
                      </button>
                    </div>

                    <div style={{ padding: "0 4px" }}>
                      <div className="section-head" style={{ fontSize: 12, marginBottom: 8 }}>🔗 快速操作</div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button className="btn btn-ghost btn-sm" style={{ flex: 1 }} onClick={() => setShareModalFor(activeReport)}>资源共享</button>
                        {activeTab === "mine" && activeReport.datasetVersionId && (
                          <button className="btn btn-ghost btn-sm" style={{ flex: 1 }} onClick={() => secureDownload(`/api/datasets/version/${activeReport.datasetVersionId}/download`, `Dataset_V${activeReport.datasetVersionId}.csv`)}>下载原始数据</button>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="empty-state" style={{ padding: "40px 0" }}>
                    ...
                    <div className="empty-title">无法查看结果</div>
                    <div className="empty-desc">该任务运行失败或结果文件已丢失</div>
                  </div>
                )}
              </div>
            ) : (
              <div className="empty-state" style={{ padding: "80px 0" }}>
                ...
                <div className="empty-title">选择一份报告</div>
                <div className="empty-desc">点击左侧列表查看分析详情、<br />进行预测或管理共享状态。</div>
              </div>
            )}
          </div>
        </div>
