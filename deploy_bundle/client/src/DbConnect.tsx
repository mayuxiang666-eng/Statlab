...
export default function DbConnect({ onDatasetCreated, onBack, addLog }: Props) {
    ...
    const loadConnections = async () => {
        const [pre, saved] = await Promise.all([
            axios.get("/api/connections/presets", { headers: getAuthHeader() }),
            axios.get("/api/connections/saved", { headers: getAuthHeader() }).catch(() => ({ data: [] }))
        ]);
        setPresets(pre.data);
    ...
    const handleTest = async () => {
        ...
        try {
            if (isCustom) {
                await axios.post("/api/connections/custom/test", customForm, { headers: getAuthHeader() });
            } else if (selectedConnId) {
                await axios.post(`/api/connections/${selectedConnId}/test`, {}, { headers: getAuthHeader() });
            }
    ...
    const goToSelect = async () => {
        ...
        if (isCustom) {
            setLoading(true);
            try {
                const res = await axios.post("/api/connections/custom/save", customForm, { headers: getAuthHeader() });
                connId = res.data.id;
        ...
        setLoading(true);
        try {
            const res = await axios.get(`/api/connections/${connId}/tree`, { headers: getAuthHeader() });
            setTree(res.data);
            setConnStatus("success");
            setStatusMsg("已连接");
            setStep("select");
    ...
    const handleRename = async () => {
        ...
        setLoading(true);
        try {
            await axios.patch(`/api/connections/${selectedConnId}`, { name: renameValue }, { headers: getAuthHeader() });
            await loadConnections();
    ...
    const handleDeleteConn = async () => {
        ...
        setLoading(true);
        try {
            await axios.delete(`/api/connections/${selectedConnId}`, { headers: getAuthHeader() });
            setSelectedConnId("");
            setPreview(null);
            setSelectedColumns([]);
            setSelectedTable("");
    ...
    const handlePreview = async (table: string) => {
        ...
        try {
            ...
            const baseRes = await axios.post(`/api/connections/${selectedConnId}/preview`, { table }, { headers: getAuthHeader() });
    ...
    const handleValidate = async () => {
        ...
        try {
            await axios.post(`/api/connections/${selectedConnId}/preview`, { table: selectedTable }, { headers: getAuthHeader() });
