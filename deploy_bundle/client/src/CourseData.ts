...
class _Step:
    def check(self): pass
step_1 = step_2 = step_3 = step_4 = step_5 = step_6 = step_7 = step_8 = _Step()
`;
...
function buildTutorialRunnableCode(cell: NotebookCell): string {
  ...
  for (const line of lines) {
    ...
    // Match line containing ____ or TODO, capture everything before it as prefix/indent
    if (/(____|TODO)/.test(line)) {
      if (!insertedRef && ref) {
        const indentMatch = line.match(/^(\s*)/);
        const indent = indentMatch ? indentMatch[1] : '';
        result.push(`${indent}# Tutorial runnable answer`);
        ref.split('\n').forEach((r) => result.push(`${indent}${r.trim()}`));
        insertedRef = true;
      }
      continue;
    }
...
step_1.check()`,
      ...
      { id: `${prefix}_e3`, type: 'markdown', content: '### Step 2: Read Excel\nUse pandas to read an in-memory Excel buffer.' },
      {
        ...
        type: 'code',
        label: 'Step 2 - Excel',
        prompt: '使用 pd.read_excel 读取 Excel 数据',
        requiresFill: true,
        placeholders: ['____'],
...
step_2.check()`,
      ...
      { id: `${prefix}_e5`, type: 'markdown', content: '### Step 3: DataFrame basics\nInspect shape and columns.' },
      {
        ...
        prompt: '查看 df_excel 的 shape 和 columns 信息',
        requiresFill: true,
        placeholders: ['____'],
...
step_4.check()`,
      ...
      { id: `${prefix}_e9`, type: 'markdown', content: '### Step 5: Summary statistics\nGenerate describe and groupby summary.' },
      {
        ...
        prompt: '使用 describe 和 groupby 统计数据',
        requiresFill: true,
        placeholders: ['____'],
...
const BLUEPRINTS: LessonBlueprint[] = [
    { stage: 1, title: 'Python Basics for Data Labs', objective: '掌握 Python 基础，包括 CSV/Excel 读取（read_csv/read_excel）、DataFrame 基础（shape/columns/info）、数据清洗初步以及 matplotlib 简单绘图。', prerequisites: ['None'], skills: ['read_csv', 'read_excel', 'DataFrame basics', 'matplotlib basics'], recommendedDataset: 'housing', difficulty: 'Beginner', duration: '20 min', tags: ['Python', 'Data I/O', 'DataFrame', 'Matplotlib'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: '', modelName: '' },
    { stage: 2, title: 'DataFrame Core Operations', objective: 'Practice filtering, sorting, missing-value handling, type conversion, and rename operations.', prerequisites: ['Lesson 1'], skills: ['filtering', 'sorting', 'dropna/fillna', 'astype', 'rename'], recommendedDataset: 'housing', difficulty: 'Beginner', duration: '22 min', tags: ['Pandas', 'DataFrame'], focusCols: ['Rooms', 'DistanceToCenter'], targetCol: 'Price', modelImport: '', modelName: '' },
    { stage: 3, title: 'Data Cleaning Practice', objective: 'Handle duplicates, simple outliers, string cleanup, and date columns step by step.', prerequisites: ['Lesson 2'], skills: ['drop_duplicates', 'string cleaning', 'datetime parsing', 'outlier basics'], recommendedDataset: 'housing', difficulty: 'Beginner', duration: '24 min', tags: ['Cleaning'], focusCols: ['Rooms', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: '', modelName: '' },
    { stage: 4, title: 'Visualization Foundations', objective: 'Create line/bar/scatter/hist charts and explain them in simple business language.', prerequisites: ['Lesson 3'], skills: ['line plot', 'bar plot', 'scatter plot', 'histogram'], recommendedDataset: 'housing', difficulty: 'Beginner', duration: '24 min', tags: ['Visualization', 'Matplotlib'], focusCols: ['Rooms', 'DistanceToCenter'], targetCol: 'Price', modelImport: '', modelName: '' },
  { stage: 5, title: 'First Model: Regression Starter', objective: 'Start modeling with train_test_split, fit/predict, and MAE evaluation.', prerequisites: ['Lesson 4'], skills: ['train_test_split', 'fit', 'predict', 'MAE'], recommendedDataset: 'housing', difficulty: 'Beginner', duration: '25 min', tags: ['Modeling'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: `from sklearn.tree import DecisionTreeRegressor`, modelName: 'DecisionTreeRegressor(random_state=42)' },
  { stage: 6, title: 'Validation and Tuning Basics', objective: 'Understand validation split, overfitting intuition, and basic parameter comparison.', prerequisites: ['Lesson 5'], skills: ['validation', 'overfitting intuition', 'parameter comparison'], recommendedDataset: 'housing', difficulty: 'Beginner', duration: '24 min', tags: ['Validation', 'Tuning'], focusCols: ['Rooms', 'DistanceToCenter'], targetCol: 'Price', modelImport: `from sklearn.tree import DecisionTreeRegressor`, modelName: 'DecisionTreeRegressor(random_state=42)' },
  { stage: 7, title: 'Underfitting vs Overfitting', objective: 'Tune model complexity and reason about generalization.', prerequisites: ['Lesson 6'], skills: ['max_leaf_nodes', 'generalization'], recommendedDataset: 'housing', difficulty: 'Intermediate', duration: '24 min', tags: ['Bias-Variance'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: `from sklearn.tree import DecisionTreeRegressor`, modelName: 'DecisionTreeRegressor(max_leaf_nodes=100, random_state=42)' },
  { stage: 8, title: 'Random Forest Regression', objective: 'Improve robustness using ensemble learning.', prerequisites: ['Lesson 7'], skills: ['RandomForestRegressor', 'ensemble methods'], recommendedDataset: 'housing', difficulty: 'Intermediate', duration: '24 min', tags: ['RandomForest'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: `from sklearn.ensemble import RandomForestRegressor`, modelName: 'RandomForestRegressor(n_estimators=120, random_state=42)' },
  { stage: 9, title: 'Feature Engineering Intro', objective: 'Design and compare engineered features.', prerequisites: ['Lesson 8'], skills: ['feature construction', 'feature comparison'], recommendedDataset: 'housing', difficulty: 'Intermediate', duration: '26 min', tags: ['FeatureEng'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: `from sklearn.ensemble import RandomForestRegressor`, modelName: 'RandomForestRegressor(n_estimators=150, random_state=42)' },
  { stage: 10, title: 'Cross Validation', objective: 'Use cross_val_score for more stable model assessment.', prerequisites: ['Lesson 9'], skills: ['KFold', 'cross_val_score'], recommendedDataset: 'housing', difficulty: 'Intermediate', duration: '28 min', tags: ['CV'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: `from sklearn.ensemble import RandomForestRegressor`, modelName: 'RandomForestRegressor(n_estimators=120, random_state=42)' },
  { stage: 11, title: 'Pipeline Workflow', objective: 'Encapsulate preprocessing and training into reusable pipelines.', prerequisites: ['Lesson 10'], skills: ['Pipeline', 'reusability'], recommendedDataset: 'housing', difficulty: 'Intermediate', duration: '28 min', tags: ['Pipeline'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: `from sklearn.ensemble import RandomForestRegressor`, modelName: 'RandomForestRegressor(n_estimators=100, random_state=42)' },
  { stage: 12, title: 'Classification Intro', objective: 'Transfer workflow from regression to classification-style tasks.', prerequisites: ['Lesson 11'], skills: ['labels', 'accuracy thinking'], recommendedDataset: 'employee', difficulty: 'Intermediate', duration: '30 min', tags: ['Classification'], focusCols: ['YearsExperience', 'PerformanceRating', 'WorkLifeBalance'], targetCol: 'Salary', modelImport: `from sklearn.ensemble import RandomForestRegressor`, modelName: 'RandomForestRegressor(n_estimators=80, random_state=42)' },
  ...
  { stage: 14, title: 'Anomaly Detection Basics', objective: 'Detect unusual behavior with threshold-driven workflows.', prerequisites: ['Lesson 13'], skills: ['outlier reasoning', 'threshold checks'], recommendedDataset: 'manufacturing', difficulty: 'Intermediate', duration: '30 min', tags: ['Anomaly'], focusCols: ['Machine', 'Operator', 'Defects'], targetCol: 'PassRate', modelImport: `from sklearn.tree import DecisionTreeRegressor`, modelName: 'DecisionTreeRegressor(max_depth=6, random_state=42)' },
  { stage: 15, title: 'Model Interpretability', objective: 'Explain which features drive model behavior.', prerequisites: ['Lesson 14'], skills: ['feature importance', 'business interpretation'], recommendedDataset: 'housing', difficulty: 'Intermediate', duration: '28 min', tags: ['Interpretability'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: `from sklearn.ensemble import RandomForestRegressor`, modelName: 'RandomForestRegressor(n_estimators=150, random_state=42)' },
  { stage: 16, title: 'Project: Home Price Estimation', objective: 'Run a full end-to-end workflow from checks to delivery.', prerequisites: ['Lesson 15'], skills: ['end-to-end project', 'delivery mindset'], recommendedDataset: 'housing', difficulty: 'Practical', duration: '35 min', tags: ['Project'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: `from sklearn.ensemble import RandomForestRegressor`, modelName: 'RandomForestRegressor(n_estimators=200, random_state=42)' },
  { stage: 17, title: 'Error Analysis and Review', objective: 'Review residuals and produce actionable improvement ideas.', prerequisites: ['Lesson 16'], skills: ['error slicing', 'improvement strategy'], recommendedDataset: 'housing', difficulty: 'Practical', duration: '30 min', tags: ['Review'], focusCols: ['Rooms', 'DistanceToCenter', 'PupilTeacherRatio'], targetCol: 'Price', modelImport: `from sklearn.ensemble import RandomForestRegressor`, modelName: 'RandomForestRegressor(n_estimators=180, random_state=42)' },
