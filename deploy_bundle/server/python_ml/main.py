...
def run_algorithm(req: MLRequest):
    ...
    try:
        ...
        elif algo == "spc.capability":
            from algorithms.spc_capability import calculate_cpk as internal_calc
            def run(payload):
                ds = payload.get("dataset", [])
                vars_map = payload.get("variables", {})
                params = payload.get("params", {})
                target = vars_map.get("target", [])[0] if vars_map.get("target") else None
                if not target:
                    return {"error": "No target variable selected"}
                return internal_calc(
                    pd.DataFrame(ds), 
                    target, 
                    usl=params.get("usl"), 
                    lsl=params.get("lsl"),
                    subgroup_size=params.get("subgroupSize", 1)
                )
        else:
...
if __name__ == "__main__":
    ...
    print("[Python ML] StatLab Python ML Service starting on port 4002...")
    uvicorn.run("main:app", host="0.0.0.0", port=4002, log_level="warning", reload=True)
